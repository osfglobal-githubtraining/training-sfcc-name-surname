...
function getSelectedOptionsUrl(optionModel, action) {
  ...
  return optionModel
    .urlSelectOptionValue(actionEndpoint, option, value)
    .toString()
}
...
function normalizeSelectedAttributes(apiProduct, params) {
  ...
  if (apiProduct.variationModel) {
    collections.forEach(
      apiProduct.variationModel.productVariationAttributes,
      function (attribute) {
...
function getConfig(apiProduct, params) {
  ...
  var promotions = PromotionMgr.activeCustomerPromotions.getProductPromotions(
    apiProduct
  )
  var optionsModel = getCurrentOptionModel(
    apiProduct.optionModel,
    params.options
  )
  var options = {
...
function getResources() {
  ...
  return {
    ...
    assistiveSelectedText: Resource.msg(
...
function showProductPage(querystring, reqPageMetaData) {
  ...
  var schemaData = require('*/cartridge/scripts/helpers/structuredDataHelper').getProductSchema(
    product
  )
