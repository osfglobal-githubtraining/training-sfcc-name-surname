...
function validateProducts(basket) {
  ...
  collections.forEach(productLineItems, function (item) {
    ...
    if (
      Object.hasOwnProperty.call(item.custom, 'fromStoreId') &&
      ...
      var availabilityLevels = item.product.availabilityModel.getAvailabilityLevels(
        item.quantityValue
      )
      result.hasInventory =
...
function validateCoupons(basket) {
  var invalidCouponLineItem = collections.find(
    basket.couponLineItems,
