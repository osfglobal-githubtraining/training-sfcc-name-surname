  ...
  collections.forEach(
    ...
    function (PLIPriceAdjustments) {
      var ItemPromo = URLUtils.url(
        ...
        'campID',
        PLIPriceAdjustments.campaignID ? PLIPriceAdjustments.campaignID : 'N/A',
        'promoID',
...
function getShippingPromoReportingURLs(shipment, reportingURLs) {
  ...
  collections.forEach(
    shipment.shippingPriceAdjustments,
...
function getBasketOpenReportingURLs(currentBasket) {
  ...
  var basketOpen = URLUtils.url(
    ...
    'MerchandizeTotal',
    formatHelpers.formatPrice(currentBasket.merchandizeTotalPrice.value),
    'AdjMerchandizeTotal',
    ...
    'AdjShippingTotal',
    formatHelpers.formatPrice(currentBasket.adjustedShippingTotalPrice.value)
  )

  reportingURLs.push(basketOpen)

  // Report all price adjustments for the entire cart
  reportingURLs = getOrderPromoReportingURLs(
    currentBasket.priceAdjustments,
    reportingURLs
  )
...
function getOrderPlacedReportingURLs(orderCount, reportingURLs) {
  ...
  result.push(
    URLUtils.url(
      ...
      'UserOrders',
      'Count',
      formatHelpers.formatNumber(orderCount)
    )
  )
...
function getOrderReportingURLs(order) {
  ...
  var orderEvent = URLUtils.url(
    ...
    StringUtils.formatCalendar(
      new Calendar(order.creationDate),
    ...
    formatHelpers.formatPrice(order.adjustedMerchandizeTotalNetPrice.value),
    'AdjMerchandizeTotalTax',
    formatHelpers.formatPrice(order.adjustedMerchandizeTotalTax.value),
    'AdjMerchandizeTotalGross',
    formatHelpers.formatPrice(order.adjustedMerchandizeTotalGrossPrice.value),
    'AdjShippingNet',
  ...
  reportingURLs = getOrderPlacedReportingURLs(
    order.customer.orderHistory.orderCount,
    reportingURLs
  )
