...
function getPrice(
  ...
  if ((product.master || product.variationGroup) && priceModel.priceRange) {
    ...
    if (
      rangePrice &&
      rangePrice.min.sales.value !== rangePrice.max.sales.value
    ) {
      return rangePrice
    }
  }
  ...
  promotionPrice = priceHelper.getPromotionPrice(
    product,
    promotions,
    currentOptionModel
  )
  ...
  if (
    promotionPrice &&
    promotionPrice.available &&
    salesPrice.compareTo(promotionPrice)
  ) {
  ...
  if (
    salesPrice.valueOrNull === null &&
    listPrice &&
    listPrice.valueOrNull !== null
  ) {
