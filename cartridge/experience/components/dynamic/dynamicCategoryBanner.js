...
module.exports.render = function (context, modelIn) {
  ...
  if (!content.category && !(content.heading && content.image)) {
    ...
    throw new Error(
      Resource.msg('pd.categoryBanner.error', 'pageDesigner', null)
    )
  }
  if (content.heading) {
  ...
  var replaceImage = content.image
    ? ImageTransformation.getScaledImage(content.image)
    : null
  model.imageUrl = replaceImage
    ? replaceImage.src.desktop
    : searchHelper.getBannerImageUrl(content.category)

  return new Template(
    'experience/components/commerce_assets/categoryBanner'
  ).render(model).text
}
