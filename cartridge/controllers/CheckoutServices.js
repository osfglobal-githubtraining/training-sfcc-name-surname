...
server.post(
  ...
  function (req, res, next) {
    ...
    var paymentProcessor = PaymentManager.getPaymentMethod(
    ...
    this.on('route:BeforeComplete', function (req, res) {
      ...
      var usingMultiShipping = req.session.privacyCache.get(
        'usingMultiShipping'
      )
      if (usingMultiShipping === true && currentBasket.shipments.length < 2) {
        req.session.privacyCache.set('usingMultiShipping', false)
...
server.post('PlaceOrder', server.middleware.https, function (req, res, next) {
  ...
  // Re-calculate the payments.
  var calculatedPaymentTransactionTotal = COHelpers.calculatePaymentTransaction(
    currentBasket
  )
