...
server.get('Page', function (req, res, next) {
  var redirectUrl =
    URLUtils.url('Page-Show').toString() + '?' + req.querystring.toString()
...
server.get('Category', function (req, res, next) {
  var redirectUrl =
    URLUtils.url('Search-Show').toString() + '?' + req.querystring.toString()
  res.redirect(redirectUrl)
  next()
})
...
server.get('Product', function (req, res, next) {
  var redirectUrl =
    URLUtils.url('Product-Show').toString() + '?' + req.querystring.toString()
  res.redirect(redirectUrl)
  next()
})
