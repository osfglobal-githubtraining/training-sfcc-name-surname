...
function getApproachingDiscounts(basket, discountPlan) {
  ...
  if (basket && basket.productLineItems) {
    ...
    approachingShippingDiscounts = discountPlan.getApproachingShippingDiscounts(
      basket.defaultShipment
    )

    orderDiscountObject = collections.map(
    ...
    shippingDiscountObject = collections.map(
...
function CartModel(basket) {
  ...
  if (basket !== null) {
    ...
    this.hasBonusProduct = Boolean(
      basket.bonusLineItems && basket.bonusLineItems.length
    )
    ...
    if (shippingModels) {
      this.shipments = shippingModels.map(function (shippingModel) {
        ...
        if (shippingModel.selectedShippingMethod) {
          result.selectedShippingMethod =
            shippingModel.selectedShippingMethod.ID
        }
