...
function getMinMaxQuantityOptions(productLineItem, quantity) {
  var availableToSell =
    productLineItem.product.availabilityModel.inventoryRecord.ATS.value
  var perpetual =
    productLineItem.product.availabilityModel.inventoryRecord.perpetual
  var max
  if (productLineItem.productInventoryListID) {
    var inventoryList = ProductInventoryMgr.getInventoryList(
      productLineItem.productInventoryListID
    )
  ...
  if (perpetual) {
    ...
    max = Math.max(
      Math.min(availableToSell, DEFAULT_MAX_ORDER_QUANTITY),
      quantity
    )
  }
