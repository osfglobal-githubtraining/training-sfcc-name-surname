    ...
    collections.forEach(
      ...
      function (bonusDiscountLineItem) {
        ...
        if (UUID === bonusDiscountLineItem.custom.bonusProductLineItemUUID) {
          ...
          bdliObj.full =
            countBonusProducts(bonusDiscountLineItem) <
            bonusDiscountLineItem.maxBonusItems
          bdliObj.maxpids = bonusDiscountLineItem.maxBonusItems
          bdliObj.url = URLUtils.url(
            'Cart-EditBonusProduct',
            'duuid',
            bonusDiscountLineItem.UUID
          ).toString()
          bdliObj.msg = bdliObj.full
            ? Resource.msg('button.bonus.select', 'cart', null)
            : Resource.msg('button.bonus.change', 'cart', null)
          result.push(bdliObj)
        }
      }
    )
