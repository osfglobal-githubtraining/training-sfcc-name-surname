...
function getOrderLevelDiscountTotal(lineItemContainer) {
  var totalExcludingOrderDiscount = lineItemContainer.getAdjustedMerchandizeTotalPrice(
    false
  )
  var totalIncludingOrderDiscount = lineItemContainer.getAdjustedMerchandizeTotalPrice(
    true
  )
  var orderDiscount = totalExcludingOrderDiscount.subtract(
    totalIncludingOrderDiscount
  )
...
function getShippingLevelDiscountTotal(lineItemContainer) {
  ...
  var totalIncludingShippingDiscount =
    lineItemContainer.adjustedShippingTotalPrice
  var shippingDiscount = totalExcludingShippingDiscount.subtract(
    totalIncludingShippingDiscount
  )
  ...
  collections.forEach(collection, function (item) {
    if (!item.basedOnCoupon) {
      result[item.UUID] = {
        ...
        type: 'promotion',
        callOutMsg:
          typeof item.promotion !== 'undefined' && item.promotion !== null
            ? item.promotion.calloutMsg
            : ''
      }
    }
  })
...
function getDiscounts(lineItemContainer) {
  ...
  collections.forEach(
    lineItemContainer.couponLineItems,
    function (couponLineItem) {
      var priceAdjustments = collections.map(
        couponLineItem.priceAdjustments,
        function (priceAdjustment) {
          return {
            callOutMsg:
              typeof priceAdjustment.promotion !== 'undefined' &&
              priceAdjustment.promotion !== null
                ? priceAdjustment.promotion.calloutMsg
                : ''
          }
        }
      )
  ...
  discounts = createDiscountObject(
    lineItemContainer.priceAdjustments,
    discounts
  )
  discounts = createDiscountObject(
    lineItemContainer.allShippingPriceAdjustments,
    discounts
  )
...
function totals(lineItemContainer) {
  if (lineItemContainer) {
    this.subTotal = getTotals(
      lineItemContainer.getAdjustedMerchandizeTotalPrice(false)
    )
    ...
    this.shippingLevelDiscountTotal = getShippingLevelDiscountTotal(
      lineItemContainer
    )
