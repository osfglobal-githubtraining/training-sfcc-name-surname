...
function getAttributes(attributeModel) {
  ...
  if (visibleAttributeGroups.getLength() > 0) {
    attributes = collections.map(
      attributeModel.visibleAttributeGroups,
      function (group) {
        var visibleAttributeDef = attributeModel.getVisibleAttributeDefinitions(
          group
        )
        var attributeResult = {}
        ...
        attributeResult.attributes = collections.map(
          ...
          function (definition) {
            ...
            if (definition.multiValueType) {
              definitionResult.value = attributeModel
                .getDisplayValue(definition)
                .map(function (item) {
                  return item
                })
            } else {
              definitionResult.value = [
                attributeModel.getDisplayValue(definition)
              ]
            }
