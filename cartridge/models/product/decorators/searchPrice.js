...
function getListPrices(hit, getSearchHit) {
  ...
  var rootPriceBook = pricingHelper.getRootPriceBook(
    priceModel.priceInfo.priceBook
  )
  ...
  try {
    ...
    if (currentApplicablePriceBooks && currentApplicablePriceBooks.length) {
      PriceBookMgr.setApplicablePriceBooks(
        currentApplicablePriceBooks.toArray()
      )
    } else {
      PriceBookMgr.setApplicablePriceBooks()
    }
  ...
  Object.defineProperty(object, 'price', {
    ...
    value: (function () {
      ...
      if (promotions.getLength() > 0) {
        var promotionalPrice = pricingHelper.getPromotionPrice(
          searchHit.firstRepresentedProduct,
          promotions
        )
