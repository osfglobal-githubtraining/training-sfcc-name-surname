...
BaseAttributeValue.prototype = {
  ...
  getUrl: function (
    ...
    if (selected) {
      url = productSearch
        .urlRelaxAttributeValue(actionEndpoint, id, value)
        .relative()
        .toString()
    } else if (!selectable) {
      url = '#'
      ...
      url = productSearch
        .urlRefineAttributeValue(actionEndpoint, id, value)
        .relative()
        .toString()
    }
  ...
  getTitle: function (selected, selectable, refinementName, displayValue) {
    ...
    return Resource.msgf(key, 'search', null, refinementName, displayValue)
  },
